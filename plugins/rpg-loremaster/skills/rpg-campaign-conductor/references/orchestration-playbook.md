...
- **`rpg-campaign-foundation`** emits → the **planning roadmap** (locked-vs-open) + handoffs to region/city/faction/location/npc, and the **"GM knows, players discover" truths** (these are the clue-mapper's primary input). Consumes → the user's seed.
...
- **`rpg-faction-creator`** emits → "rpg-npc-creator" (leader, lieutenants), "a location-creator" (the seat of power), settlements if it implies them. Consumes → a faction named by region/city/foundation + the central truth.
- **`rpg-location-creator`** emits → "rpg-npc-creator" (a named inhabitant), "rpg-faction-creator" (if it reveals an org). Consumes → a "seat of power"/site handoff from faction/city/region + party level.
