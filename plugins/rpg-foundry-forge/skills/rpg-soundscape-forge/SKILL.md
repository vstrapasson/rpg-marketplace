...
5. **Create the playlist** (for `bind-as: scene playlist`) — `create-playlist({ name, mode, fade, sounds:[{ path, volume, loop, fade, channel }] })`. Pass each track's **channel** (`music`/`environment`/`interface`) from the manifest's Channel column — for a layered `simultaneous` bed, split music vs ambience across channels so the GM gets independent live faders (`references/soundscape-policy.md`). One playlist per scene, or one campaign-ambience playlist with per-scene tracks (judgment — same reference).
