...
Present the full matrix as a go/no-go report (this is what `/forge-preflight` shows). Only proceed to the build when all CRITICAL checks pass. The local MCP patches (geometry tools, `imagePath`) are LOCAL patches to the cloned foundry-vtt-mcp repo — a `git pull`/reinstall can revert them; that's why #4/#5 are checked every run rather than assumed.
