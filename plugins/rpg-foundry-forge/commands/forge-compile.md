---
description: Compile a vault sessao/encontro/desafio into a playable Foundry session (preflight → plan → approve → build → verify → review).
---

# /forge-compile <note name>

Compile the named `sessao`, `encontro`, or `desafio` into Foundry. This hands off to the **`rpg-forge-conductor`** skill — invoke it for `<note name>` and follow its playbook:
...
3. **Build** per concern (scenes+lighting → actors+tokens → journals → encounters → challenges → ownership), serializing scene writes, updating the build manifest after each step.
